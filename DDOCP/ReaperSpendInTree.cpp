...
#define DL_ELEMENT ReaperSpendInTree

namespace
{
    const wchar_t f_saxElementName[] = L"ReaperSpendInTree";
    DL_DEFINE_NAMES(ReaperSpendInTree_PROPERTIES)
...
ReaperSpendInTree::ReaperSpendInTree() :
    ...
    m_pointsSpent(0)
{
    DL_INIT(ReaperSpendInTree_PROPERTIES)
}

DL_DEFINE_ACCESS(ReaperSpendInTree_PROPERTIES)

XmlLib::SaxContentElementInterface * ReaperSpendInTree::StartElement(
        ...
        const XmlLib::SaxAttributes & attributes)
{
    ...
    DL_START(ReaperSpendInTree_PROPERTIES)

    return subHandler;
}

void ReaperSpendInTree::EndElement()
{
    SaxContentElement::EndElement();
    DL_END(ReaperSpendInTree_PROPERTIES)
}

void ReaperSpendInTree::Write(XmlLib::SaxWriter * writer) const
{
    writer->StartElement(ElementName());//, VersionAttributes());
    DL_WRITE(ReaperSpendInTree_PROPERTIES)
    writer->EndElement();
}

size_t ReaperSpendInTree::TrainEnhancement(
...
int ReaperSpendInTree::RevokeEnhancement(
...
size_t ReaperSpendInTree::NextBuyIndex() const
{
    size_t buyIndex = 0;
...
size_t ReaperSpendInTree::Spent() const
{
    return m_pointsSpent;
}

void ReaperSpendInTree::SetSpent(size_t apsSpent)
{
